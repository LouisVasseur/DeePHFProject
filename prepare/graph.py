...
# ── NEW: arrays-based wrapper for in-memory inference (used by Streamlit app) ──

_Z_TO_SYMBOL = {1: "H", 6: "C", 7: "N", 8: "O", 9: "F", 16: "S", 17: "Cl"}


def build_graph_from_arrays(Z, coords):
    """Same as build_graph but accepts (Z, coords) numpy arrays directly.

    Writes a temporary xyz file so the openbabel + RDKit pipeline runs
    unchanged. Returns (node_features, edge_features, edge_indices) or three
    Nones if RDKit can't perceive bonds.
    """
    Z = np.asarray(Z).flatten().astype(int)
    coords = np.asarray(coords, dtype=float).reshape(-1, 3)
    assert len(Z) == len(coords), "Z and coords must have the same length"

    lines = [str(len(Z)), "from build_graph_from_arrays"]
    for z, (x, y, zc) in zip(Z, coords):
        sym = _Z_TO_SYMBOL.get(int(z))
        if sym is None:
            raise ValueError(f"Unsupported element Z={z}; supported: {sorted(_Z_TO_SYMBOL)}")
